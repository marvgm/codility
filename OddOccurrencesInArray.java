...
public class OddOccurrencesInArray {
    ...
    public static int solution(int[] A) {
        ...
        for (int valor : A) {
            listWithDuplicates.add(valor);
        }

        // TreeMap é inerentemente ordenado
        Map<Integer, Integer> mapValores= new TreeMap<>();

        listWithDuplicates.forEach(valor -> mapValores.compute(valor, (k, v) -> v == null ? 1 : ++v));

        int indice = -1;
        int valor = 1;

        for (Map.Entry<Integer, Integer> entry : mapValores.entrySet()) {
